...
def simulation(seed, n_nodes=100, family='gaussian', df=3):
    seed = int(seed)
    n_nodes = int(n_nodes)
    df = float(df)
    ...
    Y, X, params = synthetic_network(n_nodes=n_nodes, family=family, link=link,
            ...
            dispersion=dispersion, df=df, var_power=1.6, zif_prob=0.1)

    model = GLNEM(family='gaussian', link=link, n_features=8, random_state=123)
    ...
    dir_base = f'output_heavy_tails/output'
    if family == 't':
        dir_name = os.path.join(dir_base, f"{family}_{df}_n{n_nodes}")
    else:
...
for i in range(n_reps):
    simulation(seed=i, n_nodes=n_nodes, family='gaussian')

for i in range(n_reps):
    simulation(seed=i, n_nodes=n_nodes, family='laplace')

for df in [10, 5, 3]:
    for i in range(n_reps):
        simulation(seed=i, n_nodes=n_nodes, family='t', df=df)
